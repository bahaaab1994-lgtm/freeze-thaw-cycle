...
def main():
    ...
    with col1:
        selected_state = st.selectbox(
            ...
            index=0,
            help="Select the state from available options"
        )
    ...
    if st.button("Search for Freeze-Thaw Data", type="primary"):
        ...
        state_data = search_data[search_data['State'] == selected_state]
        ...
        try:
            ...
            if nearest_location is None:
                st.warning(
                    f"No monitoring stations found within 50 km of the specified coordinates in {selected_state}. "
                    "Try searching with coordinates closer to populated areas."
                )
                
                # Show available locations in the state
                st.subheader(f"Available monitoring stations in {selected_state}:")
                display_data = state_data[['County', 'Latitude', 'Longitude', 'Total_Freeze_Thaw_Cycles', 'Damaging_Freeze_Thaw_Cycles']].copy()
                st.dataframe(display_data, use_container_width=True)
                return
